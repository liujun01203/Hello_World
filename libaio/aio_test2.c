...
{  
        ...
        // 可以支持不指定O_DIRECT标志  
        // int fd = open("./helloworld.txt", O_DIRECT|O_WRONLY, S_IRWXU|S_IRWXG|S_IROTH);  
        int fd = open("./helloworld.txt", O_DIRECT|O_WRONLY);  
        printf("open: %s\n", strerror(errno));  
        ...
        iocbpp[0].u.c.nbytes = strlen(buf); // 这个值必须按512字节对齐  
        ...
        n = io_getevents(ctx, 0, 10, events, &timeout);  
